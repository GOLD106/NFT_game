...
'use strict'; {
	const DOM_COMPONENT_ID = "progress-bar";

	function StopPropagation(e) {
		e.stopPropagation()
	}
	const HANDLER_CLASS = class ProgressBarDOMHandler extends self.DOMElementHandler {
		constructor(iRuntime) {
			super(iRuntime, DOM_COMPONENT_ID)
		}
		CreateElement(elementId, e) {
			...
			elem.addEventListener("click", () => this._PostToRuntimeElementMaybeSync("click", elementId));
		...
		_GetActiveGamepads() {
			try {
				...
				if(!didLogGetGamepadsError) {
					console.warn("[Construct 3] Failed to access gamepads: ", err);
...
'use strict'; {
	const DOM_COMPONENT_ID = "list";

	function StopPropagation(e) {
		e.stopPropagation()
	}
	const HANDLER_CLASS = class ListDOMHandler extends self.DOMElementHandler {
		constructor(iRuntime) {
			super(iRuntime, DOM_COMPONENT_ID);
			this.AddDOMElementMessageHandler("set-selected-index", (elem, e) => this._OnSetSelectedIndex(elem, e["selectedIndex"]));
			this.AddDOMElementMessageHandler("add-item", (elem, e) => this._OnAddItem(elem, e));
			this.AddDOMElementMessageHandler("remove-item", (elem, e) => this._OnRemoveItem(elem, e));
			this.AddDOMElementMessageHandler("set-item", (elem, e) => this._OnSetItem(elem, e));
			this.AddDOMElementMessageHandler("clear", elem => this._OnClear(elem));
			this.AddDOMElementMessageHandler("load-state", (elem, e) => this._OnLoadState(elem, e))
		}
		CreateElement(elementId, e) {
			const isDropdown = e["isDropdown"];
			const isMultiSelect = e["isMultiSelect"];
			const items = e["items"];
			const elem = document.createElement("select");
			elem.style.position = "absolute";
			elem.style.userSelect = "none";
			elem.style.webkitUserSelect = "none";
			elem.multiple = isMultiSelect;
			if(!isDropdown) elem.size = 2;
			for(const text of items) elem.add(this._CreateOption(text));
			elem.addEventListener("touchstart", StopPropagation);
			elem.addEventListener("touchmove", StopPropagation);
			elem.addEventListener("touchend", StopPropagation);
			elem.addEventListener("mousedown", StopPropagation);
			...
			elem.addEventListener("click", e => {
				e.stopPropagation();
				this._PostToRuntimeElementMaybeSync("click", elementId, this._GetSelectionState(elem))
			});
			elem.addEventListener("dblclick", e => {
				e.stopPropagation();
				this._PostToRuntimeElementMaybeSync("dblclick", elementId, this._GetSelectionState(elem))
			});
			elem.addEventListener("change", () => this.PostToRuntimeElement("change", elementId, this._GetSelectionState(elem)));
			if(e["id"]) elem.id = e["id"];
			this.UpdateState(elem, e);
			return elem
		}
		...
		UpdateState(elem, e) {
			...
			elem.multiple = !!e["isMultiSelect"]
		}
		_OnSetSelectedIndex(elem, i) {
			elem.selectedIndex = i
		}
		_OnAddItem(elem, e) {
			const text = e["text"];
			const index = e["index"];
			const opt = this._CreateOption(text);
			if(index < 0) elem.add(opt);
			else elem.add(opt, index)
		}
		_OnRemoveItem(elem, e) {
			elem.remove(e["index"])
		}
		_OnSetItem(elem, e) {
			elem.options[e["index"]].text = e["text"]
		}
		_OnClear(elem) {
			elem.innerHTML = ""
		}
		_OnLoadState(elem, e) {
			elem.innerHTML = "";
			for(const text of e["items"]) elem.add(this._CreateOption(text));
			elem.selectedIndex = e["selectedIndex"];
			for(const index of e["selectedIndices"]) {
				const option = elem.options[index];
